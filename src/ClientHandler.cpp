...
static void on_remote_servername_resolved(int result, struct evutil_addrinfo *res, void *arg){
...
void ClientHandler::on_remote_servername_resolved(int errcode, struct evutil_addrinfo *addr){
  ...
  for (auto ai = addr; ai; ai = ai->ai_next) {
    ...
    bufferevent_setcb(remote_bev_, remote_readcb, NULL, remote_eventcb,
		      this);
...
void ClientHandler::on_read(bufferevent *bev) {
  ...
  while (!eof) {
	...
	hints.ai_family = req.addressType==Socks5Req::IPV6?AF_INET6:AF_INET;  // only ipv4
